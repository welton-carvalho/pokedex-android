...
└── route/
    ├── keys                # NavKeys serializáveis (PokemonListKey, PokemonDetailKey) + interface AppNavigator
    ├── navigation          # AppNavDisplay (monta o NavDisplay + nav entries das features), NavBackStackNavigator
...
feature/
├── pokemon-list            # Tela de listagem com Paging3 e MVI (+ modo de seleção p/ comparar)
├── pokemon-detail          # Tela de detalhe com MVI
└── pokemon-compare         # Tela de comparação de 2 Pokémon lado a lado (MVI)
```
...
core:route:navigation → core:route:keys, core:route:deeplink, feature:*
...
**Regra absoluta:** features NÃO se conhecem. `feature:pokemon-list` nunca importa nada de `feature:pokemon-detail` e vice-versa. As features dependem apenas de `core:route:keys` (NavKeys). É o módulo de montagem `core:route:navigation` que conhece as nav entries de todas as features e as compõe no `NavDisplay`.
...
Usar Navigation3 com NavKeys type-safe serializáveis definidas em `core:route:keys`:
...
Módulos de rota:
- `core:route:keys` — NavKeys + interface `AppNavigator` (`navigateTo(key)`, `navigateBack()`).
- `core:route:navigation` — `AppNavDisplay` monta o `NavDisplay`, registra as nav entries de cada feature (`pokemonListEntry`, `pokemonDetailEntry`) e expõe o `NavBackStackNavigator`.
- `core:route:deeplink` — `DeepLinkRouter` converte `Intent`/URL (`pokedex://list`, `pokedex://pokemon/{id}`, `pokedex://compare/{id1}/{id2}`) em `NavKey`.

Cada feature expõe sua própria nav entry em `feature/xxx/navigation/XxxNavEntry.kt` e usa apenas as keys de `core:route:keys` — nunca cria rotas próprias nem conhece outras features.
